...
if [[ "$OS" == 'Linux' && "$IS_RPM_DEPLOY" -eq "1" ]]; then
    ...
    for rpm in $RPMS; do
        ...
        aws s3 cp "${rpm}" s3://repo.nano.org/$BUILD/binaries/$(basename "${rpm}") --grants read=uri=http://acs.amazonaws.com/groups/global/AllUsers
        aws s3 cp "${GITHUB_WORKSPACE}/$(basename $rpm).sha256" s3://repo.nano.org/$BUILD/binaries/$(basename "${rpm}.sha256") --grants read=uri=http://acs.amazonaws.com/groups/global/AllUsers
    done
    for srpm in $SRPMS; do
        sha256sum $srpm > "${GITHUB_WORKSPACE}/$(basename ${srpm}).sha256"
        aws s3 cp "${srpm}" s3://repo.nano.org/$BUILD/source/$(basename "${srpm}") --grants read=uri=http://acs.amazonaws.com/groups/global/AllUsers
        aws s3 cp "${GITHUB_WORKSPACE}/$(basename ${srpm}).sha256" s3://repo.nano.org/$BUILD/source/$(basename "${srpm}.sha256") --grants read=uri=http://acs.amazonaws.com/groups/global/AllUsers
    done
elif [[ "$OS" == 'Linux' ]]; then
